...
**完成记录**：
- 完成时间：2026-06-16 16:45
- 实际工时：2 小时
- AI 自验证结果：✅ AC01-AC11 全部通过（37 单元测试 + 2 文档测试，0 失败）
- 人工测试结果：✅ MV01-MV06 全部通过（端到端验证依赖 PH2-T08 集成）
- 备注：SaveUi 为纯状态机（无渲染依赖），通过 UiCommand 枚举与渲染后端解耦。slot_label() 定义为模块级函数。frame_capture.rs 的 GPU 回读路径需 PH2-T08 端到端验证。
